...
event.stopPropagation()},_updateText:function(context){var activeItems=document.getElementById(context._getItemListIdentifier()).querySelectorAll('ul .active');if(activeItems.length>0){var val='';for(var i=0;i<(activeItems.length<10?activeItems.length:10);i++){val+=activeItems[i].innerText+", "}
val=val.substr(0,val.length-2);if(val.length>80){val=val.substr(0,77)+'...'}}
if(activeItems.length==document.getElementById(context._getItemListIdentifier()).querySelectorAll('ul li').length){val='Tout les items selectionnés'}
