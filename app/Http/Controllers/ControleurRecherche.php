...
class ControleurRecherche extends Controller{
 		 ...
 		 */
          public function afficherRecherche(Request $request) {
    		...
    		*/
        	$resultatRequeteSalle = Salle::select("nom_logiciel", "type_logiciel", "editeur", "siteWeb", "licence","nom_salle")
			...
			if (count($resultatRequeteLogiciel) > 0 ){
				$nomLogiciel = $resultatRequeteLogiciel[0]["nom_logiciel"] ;
				return view('accueil')->withLogiciels( $resultatRequeteLogiciel)->withRecherche($nomLogiciel);
			}
			else if( count($resultatRequeteSalle) > 0){
				$nomSalle = $resultatRequeteSalle[0]["nom_salle"] ;
				return view('accueil')->withSalles($resultatRequeteSalle)->withRecherche($nomSalle);
			}
		...
		public function afficherRechercheAdmin(Request $request){
    		...
    		*/
			...
			if (count($resultatRequeteLogiciel) > 0 ){
				$nomLogiciel = $resultatRequeteLogiciel[0]["nom_logiciel"] ;
				//return view('accueil')->withLogiciels( $resultatRequeteLogiciel)->withRecherche($nomLogiciel);
                return view('recherche_admin')->withLogiciels( $resultatRequeteLogiciel)->withRecherche($nomLogiciel);
			}
			else if( count($resultatRequeteSalle) > 0){
				$nomSalle = $resultatRequeteSalle[0]["nom_salle"] ;
				return view('recherche_admin')->withSalles($resultatRequeteSalle)->withEnseignants($enseignants)->withRecherche($nomSalle);
			}
