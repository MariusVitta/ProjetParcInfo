...
<nav class="navbar navbar-expand-lg main-navbar">
    <form class="form-inline mr-auto">
        ...
        <h1 class="font-weight-bold text-2xl text-white">{{ config('app.name', 'Gestion de logiciels') }}</h1>
    </form>
    <ul class="navbar-nav navbar-right">
        <li class="dropdown"><a href="#" data-turbolinks="false" data-toggle="dropdown" class="nav-link dropdown-toggle nav-link-lg nav-link-user">
            @if (!is_null($user))
                <div class="d-sm-none d-lg-inline-block">Bonjour, {{ $user->name }}</div></a>
            @else
                <div class="d-sm-none d-lg-inline-block">Bonjour, Bienvenue</div></a>
            @endif
